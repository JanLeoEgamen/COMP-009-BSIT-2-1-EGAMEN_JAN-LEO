...
public class WordGuess {
	...
	private void validateGuess(char letter) {
		...
		// revision here, to Prompt a message whenever it got a wrong  letter.
		if(!correct) {
			// adding message dialog using JOptionPane that prompt a message whenever it got a wrong  letter.
			JOptionPane.showMessageDialog(frame, "Incorrect guess!", "Wrong Guess", JOptionPane.ERROR_MESSAGE);
			mistakes++;
		} else {
			...
			if(displayedWord.equals(completeAnswer)) {
				...
				// adding congrats message promt 
				JOptionPane.showMessageDialog(frame, "Congratulations! You guessed it right.");
	...
	//Revision here, i revised this method and make it so that the dictionary is supplied from a CSV file.

	private void loadWords() {
							//file path of csv
		String csvFile = "C:\\Users\\Jan Leo Egamen\\eclipse-workspace\\COMP 009 BSIT2-1 EGAMEN JAN LEO\\src\\words.csv";
        String line;
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
            ...
            while ((line = br.readLine()) != null) { 
                ...
                listOfWords[index] = wordList.toArray(new String[0]);
                index++;
            }
        } catch (IOException e) {
            e.printStackTrace();
